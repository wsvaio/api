import { merge, toString, trying, dateFormat } from "@wsvaio/utils";

type middleware = (ctx: ctx, next: () => Promise<void>) => Promise<ctx>;

type ctx<custom = {}, params = {}> = {
  cache?: RequestCache;
  ...
  url?: string;
  baseURL?: string;
  body?: Partial<params> & Record<string, string> | BodyInit;
  ...
  // 执行器
  actuator?: (ctx: ctx, ...middleware: middleware[]) => Promise<ctx>;

  request?: (ctx: ctx) => Promise<ctx>;

} & custom;



export const createApi = <custom = {}, params = {}>(ctx: ctx<custom, params>) => {
  merge<ctx>(ctx, {
    headers: {},
    query: {},
    param: {},
    ...
    // 执行器，默认洋葱模型
    actuator: async (ctx: ctx, ...middleware: middleware[]) => {
      let index = -1;
    ...
    request: async (context?: ctx) => {
      if (context) {
        merge(ctx, context, { deep: Infinity });
        return ctx.request;
      }
  
      await ctx.actuator!(ctx, ...ctx.before!, ctx.core!, ...ctx.after!)
        .catch(async err => {
          ctx.error = err;
          await ctx.actuator!(ctx, ...ctx.catch!);
        })
        .finally(async () => {
          await ctx.actuator!(ctx, ...ctx.finally!);
        })
  
      return ctx;
    }

  }, { deep: Infinity, overwrite: false });
