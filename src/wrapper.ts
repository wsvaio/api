...
//   ): FinalContext<B & E, A> & Promise<FinalContext<B & E, A>>;
//   <T>(
...
export function currying<
  ...
  B extends BeforePatch = BeforePatch,
  A extends AfterPatch = AfterPatch,
>(context: BeforeContext<B, A>, ...contexts: Record<any, any>[]): Currying<E, C, B, A> {
  // function result<T extends {}>(config: string): ReturnType<typeof currying<T & E, T & C, B, A>>;
  // function result<T extends {}>(
  //   config: Omit<C, "data"> & Partial<BeforeContext<B, A>> & { config: true }
  // ): ReturnType<typeof currying<T & E, T & DeepPartial<E>, B, A>>;
  // function result<T>(
  //   config?: Omit<C, "data"> & Partial<BeforeContext<B, A>> & { config?: false }
  // ): FinalContext<B & E, A> &
  // // @ts-expect-error pass
  // Promise<IsEqual<T, unknown> extends true ? (IsEqual<E["data"], unknown> extends true ? A["data"] : E["data"]) : T>;
  // function result(
  //   config: (Partial<BeforeContext<B, A>> & { config?: boolean }) | string = {}
  // ) {
  //   if (typeof config === "string")
  //     return currying(context, ...contexts, { path: config });
  //   if (config.config === true)
